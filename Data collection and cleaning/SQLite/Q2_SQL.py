...
class HW2_sql():
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT movie_title, printf('%.2f', movie_score), cast_count FROM (SELECT title AS movie_title, score as movie_score, COUNT(cast_id) as cast_count\
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT cast_id, cast_name, printf('%.2f', average_score) FROM (SELECT cast_id, cast_name, avg(score) as average_score, COUNT(cast_name) as count \
...
where a.cast_id < b.cast_id and a.title = b.title GROUP BY cast_member_id1, cast_member_id2) as summary WHERE movie_count >=3 and average_movie_score >= 40"
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT cast_id, cast_name, printf('%.2f', avg(average_movie_score)) as collaboration_score FROM (SELECT a.cast_member_id1 as cast_id, b.cast_name, a.average_movie_score \
