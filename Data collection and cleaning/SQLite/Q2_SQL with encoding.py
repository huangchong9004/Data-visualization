...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
       ...
       ######################################################################
        with open(path, encoding="utf-8") as csv_file:
            ...
            #sql = ''
            sql1 = "INSERT INTO movies VALUES(?, ?, ?)"
            vals = []
            for row in csv_reader:
                vals.append((row[0], row[1], row[2],))
            cursor = connection.cursor()
            cursor.executemany(sql1, vals)
            connection.commit()
    ...
    def part_aii_2(self,connection, path):
        ...
        with open(path, encoding="utf-8") as csv_file:
            ...
            #sql = ''
            vals = []
            sql2 = "INSERT INTO movie_cast VALUES(?, ?, ?, ?, ?)"
            for row in csv_reader:
                vals.append((row[0], row[1], row[2], row[3], row[4],))
            cursor = connection.cursor()
            cursor.executemany(sql2, vals)
            connection.commit()
    ...
    def part_aiii(self,connection):
        ...
        part_aiii_insert_sql = "INSERT  INTO cast_bio(cast_id, cast_name, birthday, popularity) \
            SELECT cast_id, cast_name, birthday, popularity FROM (SELECT t1.cast_id, t1.cast_name, t1.birthday, t1.popularity FROM movie_cast as t1 group by cast_id) as a"
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = "SELECT printf('%.2f', SUM(CASE WHEN score > 50 AND title LIKE '%war%' THEN 1 ELSE 0 END) *100.0 / COUNT(*)) AS percentage FROM movies"
        #part_c_sql = "SELECT COUNT(a.score) FROM movies as a WHERE a.score > 50 AND a.title LIKE '%war%'"
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name, COUNT(cast_name) as count FROM movie_cast WHERE popularity > 10 GROUP BY cast_name ORDER BY count DESC LIMIT 5"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT movie_title, printf('%10.2f', movie_score), cast_count FROM (SELECT title AS movie_title, score as movie_score, COUNT(cast_id) as cast_count\
            FROM movies INNER JOIN movie_cast ON movies.id = movie_cast.movie_id GROUP BY movie_id \
                ORDER BY movie_score DESC, cast_count ASC, movie_title ASC) as a LIMIT 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT cast_id, cast_name, printf('%10.2f', average_score) FROM (SELECT cast_id, cast_name, avg(score) as average_score, COUNT(cast_name) as count \
            FROM movie_cast INNER JOIN (SELECT id, score FROM movies WHERE score > 25) as high_score_movies ON high_score_movies.id = movie_cast.movie_id \
            GROUP BY cast_id) as final WHERE final.count > 2 ORDER BY average_score DESC, cast_name ASC LIMIT 10;"
        ######################################################################
    ...
    def part_g(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_sql = "CREATE VIEW good_collaboration AS SELECT cast_member_id1, cast_member_id2, movie_count, average_movie_score FROM (SELECT a.cast_id as cast_member_id1, \
            b.cast_id as cast_member_id2, COUNT(a.title) as movie_count, avg(a.score) as average_movie_score \
            FROM (SELECT cast_id, score, title From movie_cast INNER JOIN movies ON movie_cast.movie_id = movies.id) as a, \
            (SELECT cast_id, score, title From movie_cast INNER JOIN movies ON movie_cast.movie_id = movies.id) as b \
            where a.cast_id < b.cast_id and a.title = b.title GROUP BY cast_member_id1, cast_member_id2) as summary WHERE movie_count >=3 and average_movie_score >= 40"
        ######################################################################
        return self.execute_query(connection, part_g_sql)
    
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT cast_id, cast_name, printf('%10.2f', avg(average_movie_score)) as collaboration_score FROM (SELECT a.cast_member_id1 as cast_id, b.cast_name, a.average_movie_score \
            FROM good_collaboration as a, movie_cast as b where a.cast_member_id1 = b.cast_id UNION ALL SELECT c.cast_member_id2 as cast_id, d.cast_name, c.average_movie_score \
                FROM good_collaboration as c, movie_cast as d where c.cast_member_id2 = d.cast_id) as r3 GROUP BY cast_id ORDER BY collaboration_score DESC, cast_name ASC LIMIT 5;"
        ######################################################################
    ...
    def part_h(self,connection,path):
        ...
        with open(path, encoding = 'utf-8') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter =',')
            vals = []
            sql1 = 'INSERT INTO movie_overview VALUES(?, ?)'
            for row in csv_reader:
