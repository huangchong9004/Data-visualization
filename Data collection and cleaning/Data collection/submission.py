...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        if not self.contains(id, name):
            self.nodes.append((id, name))
    
    def contains(self, id: str, name: str):
        for node in self.nodes:
            if node[0] == id and node[1] == name:
                return True
        return False

    def contains_edge(self, source: str, target: str):
        for node in self.edges:
            if (node[0] == source and node[1] == target) or (node[0] == target and node[1] == source):
                return True
        return False
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        if not self.contains_edge(source, target):
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for edge in self.edges:
            for id in edge:
                if not nodes_count.get(id):
                    nodes_count[id] = 1
                else:
                    nodes_count[id] += 1
        max_degree = max([nodes_count[k] for k in nodes_count])
        max_degree_dict = {k:nodes_count[k] for k in nodes_count if nodes_count[k] == max_degree}
        return max_degree_dict

    def single_degree_nodes(self):
        nodes_count = {}
        for edge in self.edges:
            for id in edge:
                if not nodes_count.get(id):
                    ...
                    nodes_count[id] += 1
        single_degree_nodes = len([nodes_count[k] for k in nodes_count if nodes_count[k] == 1])
        return single_degree_nodes
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        conn.request("GET","/3/movie/{}/credits?api_key={}&language=en-US".format(movie_id, self.api_key))
        r1 = conn.getresponse()
        result_string = r1.read().decode("utf-8")
        json_obj = json.loads(result_string)
        
        cast_num = min(limit, len(json_obj["cast"])) if limit != None else len(json_obj["cast"])
        #print(cast_num)
        cast_list = []
        for cast in json_obj["cast"]:
            if exclude_ids == None:
                if (int(cast['order']) < cast_num):
                    cast_list.append(cast)
            else:
                if (cast["id"] not in exclude_ids) & (int(cast['order']) < cast_num):
                    cast_list.append(cast)
        return cast_list
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        url = "api.themoviedb.org"
        conn = http.client.HTTPSConnection(url)
        conn.request("GET","/3/person/{}/movie_credits?api_key={}&language=en-US".format(person_id, self.api_key))
        r1 = conn.getresponse()
        result_string = r1.read().decode("utf-8")
        json_obj = json.loads(result_string)
        vote_threshold = vote_avg_threshold if vote_avg_threshold != None else 0.0
        movie_list = [json_obj["cast"][i] for i in range(len(json_obj["cast"])) if json_obj["cast"][i]["vote_average"] >= vote_threshold]
        #print(movie_list)
        return movie_list
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#59f5ccff-9f2a-4af1-9d28-5ae6699a4bfa"
...
if __name__ == "__main__":
    ...
    first_nodes = []
    second_nodes = []
    my_API_key = "eec9dfd55213c53928a42f363647e29b"
    tmdb_api_utils = TMDBAPIUtils(api_key = my_API_key)
    #tmdb_api_utils.get_movie_cast("500")
    movie_list = tmdb_api_utils.get_movie_credits_for_person(actor_id, 8.0)
    for i in range(len(movie_list)):
        #print(type(movie_list[i]["id"]))
        cast_list = tmdb_api_utils.get_movie_cast(str(movie_list[i]["id"]), 3)
        for cast in cast_list:
            if not graph.contains(str(cast["id"]), cast['name']):
                graph.add_node(str(cast["id"]), cast['name'])
                first_nodes.append((str(cast["id"]), cast['name']))
                graph.add_edge(str(cast["id"]), actor_id)
    ...
    #print(first_nodes)
    while loop_times < 2:
        nodes = first_nodes if loop_times == 0 else second_nodes
        #print(nodes)
        for node in nodes:
            #print(node)
            new_movie_list = tmdb_api_utils.get_movie_credits_for_person(node[0], 8.0)
            #print(new_movie_list)
            for movie in new_movie_list:
                new_cast_list = tmdb_api_utils.get_movie_cast(str(movie["id"]), 3)
                #print(new_cast_list)
                for cast in new_cast_list:
                    if not graph.contains(str(cast["id"]), cast['name']):
                        graph.add_node(str(cast["id"]), cast['name'])
                        if (loop_times == 0):
                            second_nodes.append((str(cast["id"]), cast['name']))
                    if (not graph.contains_edge(str(cast["id"]), node[0])) & (str(cast["id"]) != node[0]):
                        graph.add_edge(str(cast["id"]), node[0])
        loop_times += 1
    
    #print(graph.single_degree_nodes())
    #graph.print_nodes()
    #graph.print_edges()
    graph.write_edges_file()
    graph.write_nodes_file()
